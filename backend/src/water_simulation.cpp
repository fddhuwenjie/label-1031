...
WaterSimulation::WaterSimulation(int gridSize, float size)
    : gridSize(std::clamp(gridSize, WaterParams::MIN_GRID_SIZE, WaterParams::MAX_GRID_SIZE)), 
      ...
      totalTime(0.0f),
      rainModeEnabled(false), rainFrequency(WaterParams::DEFAULT_RAIN_FREQUENCY),
      rainAccumulator(0.0f),
      randomEngine(std::random_device{}()),
      posDist(-size / 2.0f, size / 2.0f),
      strengthDist(WaterParams::MIN_RAIN_INTENSITY, WaterParams::MAX_RAIN_INTENSITY) {
...
void WaterSimulation::setMode(WaterMode mode) {
    if (currentMode != mode) {
        ...
        if (mode == WaterMode::PRESET_ANIMATION && rainModeEnabled) {
            rainModeEnabled = false;
            LOG_INFO("Rain mode disabled due to mode switch to Preset Animation");
        }
        reset();
    }
}
...
void WaterSimulation::toggleRainMode() {
    if (currentMode == WaterMode::PHYSICS_SIMULATION) {
        ...
        LOG_INFO("Rain mode: " << (rainModeEnabled ? "ON" : "OFF"));
    } else {
        LOG_WARN("Rain mode can only be enabled in Physics Simulation mode");
    }
}

void WaterSimulation::setRainMode(bool enabled) {
    if (enabled && currentMode != WaterMode::PHYSICS_SIMULATION) {
        LOG_WARN("Rain mode can only be enabled in Physics Simulation mode");
        return;
    }
...
void WaterSimulation::setRainFrequency(float freq) {
    rainFrequency = std::max(0.1f, freq);
}

void WaterSimulation::updateRainDrops(float deltaTime) {
    if (!rainModeEnabled || rainFrequency <= 0.0f) return;
    ...
    while (rainAccumulator >= dropInterval) {
        ...
        float x = posDist(randomEngine);
        float z = posDist(randomEngine);
        float strength = strengthDist(randomEngine);
